...
Para manter o fluxo MVC estrito, os controllers acessam as instâncias compartilhadas das estruturas em `src/models/instances.ts` e delegam a formatação da resposta para a camada de views.
...
- `src/controllers`: camada responsável por tratar requisições HTTP e coordenar models e views
- `src/views`: camada responsável por formatar as respostas JSON da API
